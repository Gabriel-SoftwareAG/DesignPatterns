...
In this example, we create a `PlainCheesePizza` object representing the base pizza. We then use decorators to add tomatoes, mushrooms, or both toppings to customize the pizza. Each decorator dynamically adds the specified topping to the base pizza, resulting in a customized pizza with the desired toppings.
